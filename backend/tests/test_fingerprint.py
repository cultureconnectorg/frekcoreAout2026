...
def _ingest(frek_id, badge="CC26-BNV", ts=None):
    body = {
        "frek_id": frek_id, "event_id": "CC2026", "action": "ACTIVATION",
        "badge_type": badge, "timestamp": ts or f"2026-05-17T{secrets.randbelow(24):02d}:{secrets.randbelow(60):02d}:00Z",
        "source": "kiltikonet",
    }
...
class TestConsent:
    def test_default_all_off(self):
        ...
        for layer in ["cadence", "affinity", "device", "social", "anomaly", "coupling", "linguistic"]:
...
class TestObserve:
    def test_device_observe_requires_consent(self):
        fid = fresh_frek()
        r = requests.post(f"{API}/observe/device", json={"frek_id": fid, "raw_device_hash": "x" * 16}, timeout=5).json()
    ...
    def test_nfc_coupling_requires_consent(self):
        fid = fresh_frek()
        r = requests.post(f"{API}/observe/nfc", json={"frek_id": fid, "nfc_scan_id": "scan-1"}, timeout=5).json()
        assert r["recorded"] is False
    ...
    def test_anomaly_bot_signal_high_for_regular_cadence(self, mongo):
        ...
        for i in range(8):
            ...
            mongo.frek_events.insert_one({
                "frek_id": fid, "event_id": "TEST-ANOMALY", "action": "ACTIVATION",
                "badge_type": "CC26-BNV", "source": "kiltikonet",
...
class TestSocial:
    def test_social_copresence(self):
        ...
        for f in (f1, f2):
            body = {"frek_id": f, "event_id": shared_event, "action": "ACTIVATION",
