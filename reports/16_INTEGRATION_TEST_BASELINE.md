...
## 8. Separate CI regression found and fixed (GitHub Actions, PR #1)

Once PR #1 was opened, GitHub Actions' "Unit tests + coverage (this phase's modules)" job failed with `ModuleNotFoundError: No module named 'opentimestamps'` — a real regression: `backend/tests/test_notary_anchor_breaker.py` (new this phase) imports `notary.anchor`, which imports `opentimestamps` at module level; that package is in the real `backend/requirements.txt` but was never added to the curated `backend/requirements-ci.txt` (no prior unit test imported `notary.anchor`). Fixed by adding `opentimestamps==0.4.5` to `requirements-ci.txt`. Verified in a fresh venv built from exactly that file: `74 passed, 0 failed, 0 errors` (was `1 error during collection, 335 deselected` before the fix — collection errors are fatal to the whole pytest run, so this one line had silently broken all 67 previously-passing unit tests in CI, not just the new ones).
