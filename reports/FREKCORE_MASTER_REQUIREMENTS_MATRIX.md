...
| Identity lifecycle: renew/update | **PARTIAL** | `frek_v1` has `/renew` (`frek_v1/identity.py:256`) — still `identity_engine`-side MISSING. `identity_engine`'s `update` (display_name/metadata, distinct from `renew`) is now **IMPLEMENTED (2026-08-31)**: `PATCH /{frek_id}`, holder-initiated-by-default, refused once revoked, live-tested |
| Identity lifecycle: merge | **MISSING** | No merge logic in either system (grep negative, Phase 1 and re-confirmed); not attempted in the 2026-08-31 P1 pass either — no existing implementation in either system to model it from, a genuinely new and higher-risk capability (see `reports/FREKCORE_COMPLETION_BACKLOG.md` P1 #2) |
