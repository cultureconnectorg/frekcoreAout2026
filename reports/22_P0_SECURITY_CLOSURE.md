...
- **True per-holder ("porteur") consent authorization** does not exist for either fingerprint or geo — the ADMIN-key interim gate is a real improvement (closes the anonymous-internet-attacker case) but is not the semantically-correct long-term model. Building real owner-scoped authorization needs Contradiction C1's identity reconciliation to land first (which system's session/credential proves "you are this FREK-ID's holder"?) — tracked in `reports/FREKCORE_COMPLETION_BACKLOG.md`, not attempted here to avoid inventing identity architecture as a side effect of a security patch.
