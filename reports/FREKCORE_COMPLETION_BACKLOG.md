...
1. **Fix or gate the unauthenticated mutating routes flagged in `docs/PERMISSION_MATRIX.md`'s FLAG section**, starting with `POST /api/v1/notary/notarize` and `/anchor/*` (highest severity — arbitrary write access to the attestation chain's trigger).
...
4. **Complete the Permission Matrix's unaudited routes** (`sync_router` beyond `/baserow/webhook`, `heritage_router` beyond `/claim`, `investor_router`, `pdf_batch_router`, the 33 `frek`/`frek_router_advanced` legacy routes) — `docs/PERMISSION_MATRIX.md`'s own "Routes not fully audited this pass" section names these explicitly.
